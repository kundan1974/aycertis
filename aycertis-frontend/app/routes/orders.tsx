...
function SalesOrdersTab() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError("")
    fetch("/api/inventory/sale-orders/", {
  ...
  const filteredOrders = orders.filter((order) => {
    const customerText =
      ...
      (order.customer && typeof order.customer === "string" ? order.customer : "")
    const matchesSearch =
      !search ||
      (customerText && customerText.toLowerCase().includes(search.toLowerCase())) ||
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
        <div>
          ...
          <p className="text-slate-600 mt-2">View, create, and manage sales orders placed by customers</p>
        </div>
      ...
      <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-lg border border-blue-100/50 overflow-hidden">
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50/80 to-green-50/80 border-b border-blue-100/50">
          <h3 className="text-lg font-semibold text-slate-700 flex items-center gap-2">
            ...
            Sales Orders ({filteredOrders.length} items)
          </h3>
        </div>
                ...
                <tr>
                  <td colSpan={8} className="px-4 py-8 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-slate-500 text-sm">No sales orders found</span>
